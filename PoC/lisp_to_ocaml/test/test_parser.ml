...
let test_parse_bool_true () =
  ...
  let result = parse input in
  match result with
  | Ok [ Decl (Def (Val (Bind _), Bool true)) ] ->
...
let test_parse_bool_false () =
  ...
  let result = parse input in
  match result with
  | Ok [ Decl (Def (Val (Bind _), Bool false)) ] ->
...
let test_parse_simple_function () =
  ...
  let result = parse input in
  match result with
  | Ok [ Decl (Def (Func (_, [ Bind _ ], T.Inferred), FnAp _)) ] ->
...
let test_parse_function_with_type () =
  ...
  let result = parse input in
  match result with
  | Ok [ Decl (Def (Func (_, [ Bind _ ], T.Int), FnAp _)) ] ->
...
let test_parse_function_with_typed_param () =
  ...
  let result = parse input in
  match result with
  | Ok [ Decl (Def (Func (_, [ TypedBind (_, T.Int) ], T.Int), FnAp _)) ] ->
...
let test_parse_let_simple () =
  ...
  let result = parse input in
  match result with
  | Ok [ Expr (Let ([ (Val (Bind _), Int 10) ], Sym _)) ] ->
...
let test_parse_let_multiple_bindings () =
  ...
  let result = parse input in
  match result with
  | Ok [ Expr (Let ([ (Val (Bind _), Int 10); (Val (Bind _), Int 20) ], FnAp _)) ] ->
...
let test_parse_let_with_type () =
  ...
  let result = parse input in
  match result with
  | Ok [ Expr (Let ([ (Val (TypedBind (_, T.Int)), Int 10) ], Sym _)) ] ->
...
let test_parse_let_function () =
  ...
  let result = parse input in
  match result with
  | Ok [ Expr (Let ([ (Func (_, [ Bind _ ], T.Inferred), FnAp _) ], FnAp _)) ] ->
...
let test_parse_fn_simple () =
  ...
  let result = parse input in
  match result with
  | Ok [ Expr (Fn ([ Bind _ ], T.Inferred, Sym _)) ] ->
...
let test_parse_fn_with_typed_param () =
  ...
  let result = parse input in
  match result with
  | Ok [ Expr (Fn ([ TypedBind (_, T.Int) ], T.Inferred, Sym _)) ] ->
...
let test_parse_match_simple () =
  ...
  let result = parse input in
  match result with
  | Ok [ Expr (Match (Sym _, [ (Int 0, Int 1); (Wildcard, Int 2) ])) ] ->
...
let test_parse_match_list () =
  ...
  let result = parse input in
  match result with
  | Ok [ Expr (Match (Sym _, [ (List [], Int 0); (Cons (Bind _, Bind _), Int 1) ])) ] ->
...
let test_parse_int_type () =
  ...
  let result = parse input in
  match result with
  | Ok [ Decl (Def (Val (TypedBind (_, T.Int)), Int 1)) ] ->
...
let test_parse_list_type () =
  ...
  let result = parse input in
  match result with
  | Ok [ Decl (Def (Val (TypedBind (_, T.List T.Int)), List [])) ] ->
