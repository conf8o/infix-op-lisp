...
let test_parse_bool_false () =
  ...
  let result = parse input in
  match result with
  | Decl (Def (Val (Bind _), Bool false)) ->
...
let test_parse_empty_list () =
  ...
  match result with
  | Expr (List []) ->
    Alcotest.(check pass) "parse empty list" () ()
...
let () =
  ...
  run "Parser tests" [
    ...
    "function", [
      ...
      test_case "parse function with typed param" `Quick test_parse_function_with_typed_param;
    ];
    "let", [
      test_case "parse simple let" `Quick test_parse_let_simple;
      test_case "parse let with multiple bindings" `Quick test_parse_let_multiple_bindings;
    ...
    "if", [
      test_case "parse if" `Quick test_parse_if;
    ];
    "match", [
