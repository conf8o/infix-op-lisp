...
let return (value : 'a) : 'a parser =
  fun input -> Success (value, input)
...
let optional (p : 'a parser) : 'a option parser =
  (let* value = p in return (Some value))
  <|> return None
  ...
  fun input ->
    if String.length input = 0 then
      ...
      if pred c then
        ...
        Failure (Printf.sprintf "Character '%c' does not satisfy predicate" c, { line = 0; column = 0 })

let char (expected : char) : char parser =
  satisfy (fun c -> c = expected)
...
let integer : int parser =
  ...
  return (match sign with Some _ -> -num | None -> num)

let identifier : string parser =
  let* first = letter <|> satisfy (fun c ->
    c = '_' || c = '+' || c = '-' || c = '*' || c = '/' || c = '=' || c = '<' || c = '>') in
...
let rec parse_type () : T.lisp_type parser =
  skip_whitespace (
    (keyword "Int" >>= fun _ -> return T.Int)
...
and parse_arrow_type () : T.lisp_type parser =
  ...
  let rec parse_arrow_parts acc =
    ...
    | None ->
      ...
      | _ -> return (List.fold_right (fun t1 t2 -> T.Arrow (t1, t2)) (List.tl all_types) (List.hd all_types))
  in
  parse_arrow_parts []
...
let rec parse_pattern () : patt parser =
  skip_whitespace (
    (keyword "true" >>= fun _ -> return (bool_patt true))
...
let rec parse_expr () : lisp_expr parser =
  skip_whitespace (
    (keyword "true" >>= fun _ -> return (Bool true))
...
and parse_let_bindings () : binding list parser =
  many (parse_let_binding ())
  ...
  skip_whitespace (
    (let* _ = lexeme (char '(') in
     ...
     | None ->
       ...
       let* _ = lexeme (char ')') in
       let* type_opt = optional (let* _ = lexeme (char ':') in parse_type ()) in
       let* body = parse_expr () in
       let ret_type = match type_opt with Some t -> t | None -> T.Inferred in
...
let parse_lisp () : lisp parser =
  skip_whitespace (
    (let* decl = parse_def () in return (Decl decl))
    <|>
    (let* expr = parse_expr () in return (Expr expr))
  )
