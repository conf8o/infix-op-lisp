...
- 2026-03-15 開始
- 2026-03-20 ARCHIVED. OCamlなどに倣い、あらゆる構文において、冗長でも、充足していなくてもパターンによる束縛と型注釈を許す。
...
(match lst
  ...
  _ error)
```
def, letのbindingにmatchをうまく追加したい。
ただし、関数定義とmatch束縛は区別する。
関数定義とパターンマッチが曖昧にならないように、パターンマッチについては明確な判断基準を設ける。Haskellによせる（Upper case、特定の記号など）

matchへの脱糖はlisp上では行わない。ocamlにそのままトランスパイルする。なので、実行時エラーになるはず。
型検査は、judge_let_type内で、judge_match_typeを利用したい。
...
Phase 1, Phase2, Phase3 全部について、

- 束縛パターンを整理することによってパターンはまとめて拡張できると思う。
- 型推論については慎重になりたい。
  - OCamlではパターン内で型注釈が可能なのでそれを採用したい。
  - `let ((x : int), (y : int)) = (1, 10) in ...`
- なのでやはりlisp_astの束縛パターン整理が先決かも。
